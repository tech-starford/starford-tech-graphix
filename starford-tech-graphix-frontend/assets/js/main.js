  ...
  try {
    ...
    if (likesRes.ok) {
      ...
      localStorage.setItem(
        "likedTemplates",
        JSON.stringify(Array.from(likedTemplates)),
      );
    }
    if (favsRes.ok) {
      ...
      localStorage.setItem(
        "favoritedTemplates",
        JSON.stringify(Array.from(favoritedTemplates)),
      );
    }
    if (bookmarksRes.ok) {
      ...
      localStorage.setItem(
        "bookmarkedTemplates",
        JSON.stringify(Array.from(bookmarkedTemplates)),
      );
    }
  } catch (err) {
...
async function toggleLike(templateId) {
  ...
  try {
    const res = await fetchWithAuth(
      `${API_BASE_URL}/api/interactions/like/${templateId}`,
      {
        method: "POST",
      },
    );
...
async function toggleFavorite(templateId) {
  ...
  try {
    const res = await fetchWithAuth(
      `${API_BASE_URL}/api/interactions/favorite/${templateId}`,
      {
        method: "POST",
      },
    );
...
async function toggleBookmark(templateId) {
  ...
  try {
    const res = await fetchWithAuth(
      `${API_BASE_URL}/api/interactions/bookmark/${templateId}`,
      {
        method: "POST",
      },
    );
...
async function downloadTemplate(templateId) {
  try {
    const res = await fetchWithAuth(
      `${API_BASE_URL}/api/interactions/download/${templateId}`,
      {
        method: "POST",
      },
    );
...
async function shareTemplate(templateId) {
  try {
    const res = await fetchWithAuth(
      `${API_BASE_URL}/api/interactions/share/${templateId}`,
      {
        method: "POST",
      },
    );
...
function updateLikeCount(templateId, count) {
  document
    .querySelectorAll(`.like-count[data-id="${templateId}"]`)
    .forEach((el) => {
...
function updateLikeIcon(templateId, liked) {
  document
    .querySelectorAll(`.like-icon[data-id="${templateId}"]`)
    .forEach((icon) => {
      icon.className = liked
        ? "fas fa-heart like-icon active"
...
function updateFavoriteCount(templateId, count) {
  document
    .querySelectorAll(`.favorite-count[data-id="${templateId}"]`)
    .forEach((el) => {
      el.textContent = count;
    });
  if (window.currentTemplateId == templateId) {
...
function updateFavoriteIcon(templateId, favorited) {
  document
    .querySelectorAll(`.favorite-icon[data-id="${templateId}"]`)
    .forEach((icon) => {
...
function updateBookmarkCount(templateId, count) {
  document
    .querySelectorAll(`.bookmark-count[data-id="${templateId}"]`)
    .forEach((el) => {
      el.textContent = count;
    });
  if (window.currentTemplateId == templateId) {
...
function updateBookmarkIcon(templateId, bookmarked) {
  document
    .querySelectorAll(`.bookmark-icon[data-id="${templateId}"]`)
    .forEach((icon) => {
      icon.className = bookmarked
        ? "fas fa-bookmark bookmark-icon active"
        : "far fa-bookmark bookmark-icon";
    });
  if (window.currentTemplateId == templateId) {
...
function updateDownloadCount(templateId, count) {
  document
    .querySelectorAll(`.download-count[data-id="${templateId}"]`)
    .forEach((el) => {
      el.textContent = count;
    });
  if (window.currentTemplateId == templateId) {
...
function updateShareCount(templateId, count) {
  document
    .querySelectorAll(`.share-count[data-id="${templateId}"]`)
    .forEach((el) => {
      el.textContent = count;
    });
  if (window.currentTemplateId == templateId) {
...
window.openTemplateModal = async function (id) {
  try {
    ...
    document.getElementById("modalCategory").textContent =
      template.subcategory || template.category;
    document.getElementById("modalFileFormat").textContent =
      template.file_formats ? template.file_formats.join(", ") : "";
    ...
    document.getElementById("modalSoftware").textContent = template.software
      ? template.software.join(", ")
      : "";
    document.getElementById("modalPrintReady").textContent =
      template.print_ready || "";
    document.getElementById("modalColorMode").textContent =
      template.color_mode || "";
    document.getElementById("modalFileSize").textContent =
      template.file_size || "";
    document.getElementById("designDetails").textContent =
      template.design_details || "";
    document.getElementById("materialsSpecs").textContent =
      template.materials_specs || "";
    document.getElementById("designInspiration").textContent =
      template.design_inspiration || "";
    document.getElementById("practicalApplications").textContent =
      template.practical_applications || "";
    document.getElementById("modalBadge").textContent =
      template.badge || "Popular";
    document.getElementById("modalFullImage").src =
      template.full_image_url || template.image_url;

    // Format badges
    const formatBadges = document.getElementById("formatBadges");
    if (formatBadges) {
      formatBadges.innerHTML = (template.file_formats || [])
        .map((f) => `<span class="format-badge">${f}</span>`)
        .join("");
    }
    ...
    if (colorPalette) {
      colorPalette.innerHTML = (template.colors || [])
        .map(
          (c) =>
            `<div class="color" style="background:${c}" title="${c}"></div>`,
        )
        .join("");
    }
    ...
    if (tagsContainer) {
      tagsContainer.innerHTML = (template.tags || [])
    ...
    if (featuresList) {
      featuresList.innerHTML = (template.features || [])
        .map((f) => `<li>${f}</li>`)
        .join("");
    }
    ...
    document.getElementById("modalLikeBtn").innerHTML =
      `<i class="${liked ? "fas" : "far"} fa-heart"></i><span class="action-count" id="modalLikeCount">${template.likes_count || 0}</span>`;
    document.getElementById("modalFavoriteBtn").innerHTML =
      `<i class="${favoritedTemplates.has(template.id) ? "fas" : "far"} fa-star"></i><span class="action-count" id="modalFavoriteCount">${template.favorites_count || 0}</span>`;
    document.getElementById("modalBookmarkBtn").innerHTML =
      `<i class="${bookmarkedTemplates.has(template.id) ? "fas" : "far"} fa-bookmark"></i><span class="action-count" id="modalBookmarkCount">${template.bookmarks_count || 0}</span>`;
    document.getElementById("modalShareBtn").innerHTML =
      `<i class="far fa-share-alt"></i><span class="action-count" id="modalShareCount">${template.shares_count || 0}</span>`;
    document.getElementById("modalDownloadBtn").innerHTML =
...
function setupSearch() {
  ...
  searchInput.addEventListener(
    ...
    debounce(async function () {
      ...
      const categoryMatches = Object.entries(categoryLinks)
